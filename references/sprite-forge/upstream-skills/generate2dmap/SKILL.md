...
Before doing any work for a request that may create, revise, scaffold, export, or wire visible game assets, determine whether the user has explicitly chosen the asset source. This check happens at the very start of the skill, before inspecting the target game, planning the pipeline, generating manifests, running scripts, creating artifacts, or touching files.

If the asset source is missing or ambiguous, stop immediately and ask one concise question: "Do you want polished assets with `image_gen`, or fast validation assets with `local-svg` for this run?" Do not proceed until the user answers.

Only skip this gate when the user's wording clearly specifies the mode, such as "精美资产", "正式美术", "use image_gen", "imagegen", "快速验证", "快速原型", or "local-svg". Record polished/final-art choices as `visual_asset_source: image_gen`; record fast-validation/prototype choices as `visual_asset_source: local_svg`. Follow that choice for the entire run.
...
This skill supports image-generation-first visual assets after the `Mandatory Start Gate` confirms that the user wants final generated art. When `visual_asset_source` is `image_gen`, use built-in `image_gen` as the creative art source for base maps, in-world reference mockups, dressed references, stage references, prop sheets, prop sprites, tileset art, parallax layers, battle backgrounds, and other visible map assets.

### Asset Source Confirmation

The `Mandatory Start Gate` is required before any planning, file changes, script execution, manifest generation, asset generation, copying, or wiring. Do not silently default to `image_gen`, `local-svg`, mock PNGs, existing assets, or any provider-specific shortcut.
...
Only use `local-svg` generated validation art when the `Mandatory Start Gate` has confirmed fast-validation mode. If using an engine target such as `Godot_TileMap`, generate or reuse the visual tileset art first, then use scripts/code only to build tile layers, collision, zones, and scene wiring.
...
Use this workflow for final-quality Web RPG/top-down RPG maps when actors move on a generated still background. Do not guess blocker polygons first. Boundaries must come from a positive walkable-area mask: cyan/walkable pixels mean the player may stand there, and everything outside the accepted mask becomes blocked after inversion.

1. Before final map generation, define a required walk graph: starts, hubs, bridges, stairs, platforms, exits, NPC/event spaces, transfer targets, and route edges that must be reachable in one connected network. This walk graph is the source of truth for gameplay reachability.
2. Generate and accept the still map image first. The prompt must make the intended walk graph broad, readable, and connected. Do not assume material names decide walkability: flower beds, petals, clover, shallow water, lily pads, grass, moss, and leaf surfaces may be walkable when the route design says they are; walls, deep water, cliffs, props, furniture, posts, fences, and outer frame space are blocked unless explicitly designed as traversal surfaces.
3. Treat the still background as a visual QA checkpoint before boundary generation. Identify any route surfaces that are visually easy to miss or misread, such as flower interiors behind rims, petal ledges, mossy or clover platforms, shallow-water paths, lily pads, stairs partly hidden by leaves, bridges under foreground grass, exits at the frame edge, and traversal surfaces covered by shadows or perspective overlap. Also identify visually tempting blocked areas such as deep water, dense foliage, rocks, prop clusters, flower walls, outer frame space, and decorative foreground cover. This checkpoint informs the boundary prompt but does not create a separate pre-QA artifact.
4. After accepting the still background, make it visible in conversation context, then use built-in `image_gen` or image editing directly to create a same-aspect walkable path boundary mask. The prompt should ask the model to mark the actual visible player-walkable paths and traversal surfaces in bright cyan at about 50% opacity. The prompt must explicitly name the required walk graph and the easy-to-miss walkable surfaces from the visual checkpoint, plus the tempting but blocked surfaces to avoid. Do not require a separate pre-QA prompt artifact, and do not make exact coordinates the imagegen source of truth.
5. The mask must preserve the map layout and mark only actual player-walkable regions. It must keep the required walk graph continuous, including walkable flower beds, petals, clover, shallow water, lily pads, grass, moss, or leaf surfaces when the design says those areas are walkable. If a flower rim, arch, canopy, character, prop, shadow, foreground object, or perspective overlap visually covers a road but does not truly block gameplay, bridge the cyan mask through/under that occluder so both sides of the route remain connected. Do not mark blocked regions directly, and do not add labels, outlines, grids, arrows, text, UI, or debug symbols.
6. Extract cyan locally into a binary walkable mask. Use deterministic thresholding tolerant of imperfect cyan, denoise, close small gaps, remove tiny islands, and keep the main connected walkable component.
7. Validate the required walk graph against the extracted mask. If a required bridge, stair, platform, exit, or route point is disconnected, repair with a corridor following the intended visible traversal surface or the implied route under a non-blocking visual occluder. The repair may cross flowers, grass, clover, shallow water, lily pads, petals, moss, leaves, shadows, decorative overlays, or foreground perspective cover when those surfaces are part of the actual route; it must not spill into props, walls, deep water, cliffs, furniture, posts, fences, or other truly blocked scenery.
8. Invert the accepted walkable mask into blocked regions, vectorize those blocked regions into pixel-coordinate `collision_shapes`, and keep the source mask, extracted mask, and walkable overlay as QA sidecars.
9. Export a red boundary/collision preview over the accepted background, validate start positions, transfers, NPCs, battle/event points, bridges, stairs, and platforms are inside the same walkable component, then wire the boundary file into the map metadata.
10. Generate dynamic/I2V map backgrounds only after the still image and mask-derived boundaries pass validation. Dynamic media must preserve roads, bridges, stairs, exits, scale, camera, and collision-relevant terrain exactly.

### Top-Down RPG Background-to-Boundary Flow

For top-down RPG maps, use this complete generation flow:

1. Select the map mode and target resolution/aspect ratio, then define the camera angle, scale, art direction, and whether the runtime uses a flat still, layered objects, or y-sorted foreground occluders.
2. Define the walk graph before any image generation: start position, all exits/transfers, main hub, optional side pockets, NPC/event/battle spaces, bridge/stair/platform segments, and route edges. Mark every required surface as walkable or blocked by gameplay intent, not by material name.
3. Write the still-background prompt from the walk graph. It must request broad connected traversal surfaces, visible entrances and exits, clear route continuity, no gameplay props on required routes, and enough empty standing space around interactables and transfers.
4. Generate the still background with `image_gen`, save it, normalize dimensions if needed, and store the prompt with the asset.
5. View the accepted background and run a visual checkpoint in the agent's reasoning: list easy-to-miss walkable surfaces, tempting blocked surfaces, occluders that should be bridged through, and route nodes that must remain connected. This is not a separate artifact unless the user asks for one.
6. Make the accepted background visible immediately before boundary generation. Generate a same-aspect cyan walkable mask with `image_gen` or image editing. The prompt must preserve the exact map layout, mark only walkable surfaces, explicitly include the walk graph and easy-to-miss walkable areas, explicitly exclude tempting blockers, and avoid labels/UI/coordinates.
7. Extract cyan into a binary walkable mask, denoise, fill small gaps, keep the main component, and repair only against the required walk graph.
8. Invert the final walkable mask into collision shapes, export QA sidecars (`source`, `extracted`, `walkable overlay`, and red collision preview), wire `walkable_mask_ref` and `boundary_source` into metadata, and run boundary validation before dynamic backgrounds or final build export.
...
- `visual_asset_source`: image_gen | local_svg
...
- Do not infer `visual_asset_source`. The `Mandatory Start Gate` must determine whether the user wants polished `image_gen` assets or fast-validation `local-svg` assets before any work begins.
...
2. Choose the pipeline axes.
   ...
   - Select `visual_asset_source` only after the user has explicitly chosen polished `image_gen` assets or fast-validation `local-svg` assets. If it is missing or ambiguous, the `Mandatory Start Gate` should already have stopped the skill before target inspection, planning, file changes, script execution, or artifact generation.
   - Treat `hybrid` as a result of combining axes, not as a primary category.

3. Produce assets.
   - For `visual_asset_source: image_gen`, write the creative prompts manually and use built-in `image_gen` for visible map art. For `visual_asset_source: local_svg`, use the project's local SVG asset generator only for fast validation.
   - For baked raster maps, generate one background with built-in `image_gen` in polished mode, or one local SVG background in fast-validation mode, then add optional collision/zones metadata.
   ...
   - Do not present a rerunnable script that creates the whole art pack as the main solution unless the user chose fast-validation `local-svg` mode.
...
4. Build metadata.
   ...
   - For final-quality Web RPG/top-down RPG still maps, derive `collision_shapes` from the accepted walkable mask by inversion, not from hand-guessed blocker polygons. Store mask QA sidecars and boundary provenance such as `walkable_mask_ref` and `boundary_source` when the target schema supports them.
...
5. Validate and preview.
   ...
   - For walkable-mask boundaries, validate that required starts, exits, transfer targets, bridges, stairs, platforms, NPC/event points, and walkable hints are in bounds, outside collision, and reachable within the same extracted walkable component. Export both the cyan walkable preview and the red blocked/collision preview.
...
- no script-drawn final tileset art unless the user chose fast-validation `local-svg` mode
