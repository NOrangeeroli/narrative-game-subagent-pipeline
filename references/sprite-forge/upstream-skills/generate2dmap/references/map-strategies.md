...
Do not choose a visual asset source silently. If the user has not explicitly chosen polished assets or fast-validation assets, stop at the start of the skill before target inspection, pipeline planning, file changes, script execution, manifest generation, or asset generation, and ask them to choose. Polished assets use `image_gen`; fast-validation assets use `local-svg`. Base maps, in-world reference mockups, dressed references, stage references, prop sheets, prop sprites, tileset art, parallax layers, and battle backgrounds should come from the user-confirmed mode.
