...
description: "Generate and postprocess general 2D game assets and animation sheets after confirming the user's visible asset source: pixel-art sprites, clean HD map props, creatures, characters, NPCs, spells, projectiles, impacts, props, summons, and transparent GIF exports. Use when Codex should infer the asset plan from a natural-language request, match a reference or map art style, call built-in `image_gen` for solid-magenta raw sheets when chosen, and use the local processor only for chroma-key cleanup, frame extraction, alignment, QC, and transparent exports."
---
...
Before doing any work for a request that may create, revise, scaffold, export, or wire visible sprite/prop assets, determine whether the user has explicitly chosen the asset source. This check happens at the very start of the skill, before inferring the asset plan, generating manifests, running scripts, creating layout guides, processing images, creating artifacts, or touching files.

If the asset source is missing or ambiguous, stop immediately and ask one concise question: "Do you want polished sprite assets with `image_gen`, or fast validation assets with `local-svg` for this run?" Do not proceed until the user answers.
...
- The `Mandatory Start Gate` is required before any planning, file changes, script execution, manifest generation, layout guide creation, image processing, asset generation, copying, or wiring. Do not silently default to `image_gen`, `local-svg`, mock PNGs, existing assets, or any provider-specific shortcut.
...
- When the confirmed asset source is `image_gen`, use built-in `image_gen` for every raw image.
- In polished mode, do not create raw sprite art with Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots; requested sprite art must originate from built-in `image_gen`. In fast-validation mode, use the project's `local-svg` output only as validation art.
...
Use built-in `image_gen` only when the `Mandatory Start Gate` has confirmed `image_gen` as the asset source.
