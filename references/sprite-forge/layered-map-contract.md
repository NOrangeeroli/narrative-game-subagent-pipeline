...
8. `boundary-preview`: QA-only overlay of collision metadata on the accepted
   base/background. This is not runtime art and must not be used as the map
   background.
...
Make collision-relevant terrain boundaries visually readable, but do not draw
debug collision colors, masks, outlines, arrows, labels, grid coordinates, or
annotation overlays into the production base map. Export those as metadata and
QA previews after the map image exists.
For Web RPG maps, treat walkable path design as the source of truth: define or
infer a connected walk graph first, generate the background so tan roads,
bridges, stairs, and platforms follow that graph, then generate a separate cyan
walkable mask from visual QA. The mask marks where the player can walk; the
blocked boundary is the inverse of the accepted mask.

For final Web RPG map stills, prefer pixel-native runtime metadata:
...
Use the same pixel coordinate system for start positions, events, transfer
targets, walkable hints, collision rectangles/polygons, and QA boundary
previews. Do not create a per-pixel collision grid; collision remains vector
metadata.
```

## Walkable Mask Boundary Pass

Run this pass after accepting the Sprite Forge still map and before I2V or
dynamic background generation:

1. Verify that all intended roads, bridges, stairs, platforms, exits, and
   interaction spaces are visually connected in the still image.
2. Generate a same-layout walkable mask image directly from the accepted still
   background with imagegen/editing: bright cyan at about 50% opacity
   over actual player-walkable surfaces from the gameplay route/walk graph,
   including roads, bridges, stairs, platforms, clearings, interaction spaces,
   and any intended walkable flower beds, petals, clover, shallow water, lily
   pads, grass, moss, or leaf surfaces. If a flower rim, arch, canopy,
   character, prop, shadow, foreground object, or perspective overlap visually
   covers a route but is not a true gameplay blocker, bridge the cyan mask
   through/under it so both sides remain connected. Do not mark truly blocked
   props, walls, deep water, cliffs, furniture, posts, fences, or outer frame
   space.
4. Extract the cyan region locally into a binary mask. Denoise, remove tiny
   islands, close small gaps, and keep the main walkable component.
5. Enforce required route connectivity from the walk graph with narrow
   centerline corridors if needed. This is a gameplay repair step, not a visual
   effect.
6. Invert the final walkable mask to get blocked regions, vectorize those
   regions into `collision_shapes`, then export both cyan walkable preview and
   red boundary preview for QA.
7. Only use I2V after the still image and mask-derived boundaries validate.
   Dynamic backgrounds must preserve roads, bridges, stairs, platforms, and
   collision-relevant terrain exactly.
