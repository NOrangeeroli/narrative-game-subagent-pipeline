...
motion.<base_asset_id>.walk
motion.<base_asset_id>.walk.down.1
```
...
Player and NPC movement must visibly switch from idle to walking. The reusable
fallback is `motion.<sprite>.walk`, generated by
`scripts/generate_runtime_media.py` from accepted sprite stills. Final-quality
player movement still prefers Sprite Forge walk animation: generate a 4x4
four-direction walk sheet or equivalent directional frame set for the
controllable actor, bind it as a runtime asset, and set the actor's
`walk_sheet_asset_id` or `walk_frame_asset_ids` so movement uses multi-frame
walking art. NPCs that move in `scene-scripts.json` must at minimum have a
`sprite_asset_id` that resolves to a still sprite so the fallback walking GIF or
runtime walking class can play during scripted movement.
