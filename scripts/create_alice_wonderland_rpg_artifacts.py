...
def write_rpg_tables(run_root: Path) -> None:
    ...
    write_json(rpg / "rpg-campaign.json", {
        "title": "爱丽丝奇遇记",
        "start_map_id": "map.rabbit_hole_garden",
        ...
        "entry_points": [{
            "id": "entry.alice",
            "title": "好奇的爱丽丝",
            "description": "以爱丽丝视角探索兔洞花园、疯帽匠茶会和红心法庭。",
            "start_map_id": "map.rabbit_hole_garden",
            "start_position": {"x": 2, "y": 5},
            "party": ["actor.alice"],
            "initial_quests": ["quest.follow_rabbit"],
        ...
        "required_assets": [
            "map.rabbit_hole_garden", "map.mad_tea_party", "map.queen_court",
            "tileset.wonderland", "sprite.alice", "sprite.white_rabbit", "sprite.cheshire_cat",
    ...
    write_json(rpg / "world-map.json", {"title": "梦境国度", "start_map_id": "map.rabbit_hole_garden", "maps": [
...
def write_maps(run_root: Path) -> None:
    ...
    garden = [(x, 5) for x in range(1, 13)] + [(6, y) for y in range(2, 8)] + [(3, 4), (9, 4)]
    tea = [(x, 5) for x in range(1, 13)] + [(4, y) for y in range(2, 8)] + [(9, y) for y in range(2, 8)]
    court = [(x, 5) for x in range(1, 13)] + [(7, y) for y in range(1, 8)] + [(4, 4), (10, 4)]
    write_json(maps / "rabbit_hole_garden.map.json", {
        "id": "map.rabbit_hole_garden", "title": "兔洞花园", "width": 14, "height": 9, "asset_id": "map.rabbit_hole_garden", "boundary_file": "../boundaries/rabbit_hole_garden.boundaries.json",
        "layers": {"ground": make_path_layer(14, 9, garden, "grass"), "collision": collision(14, 9)},
        "events": [
            {"id": "npc.white_rabbit", "type": "npc", "x": 3, "y": 5, "name": "白兔先生", "dialogue_id": "dialogue.rabbit", "sprite_asset_id": "sprite.white_rabbit", "quest_id": "quest.follow_rabbit"},
            {"id": "npc.cheshire_cat", "type": "npc", "x": 6, "y": 3, "name": "柴郡猫", "dialogue_id": "dialogue.cat", "sprite_asset_id": "sprite.cheshire_cat"},
            {"id": "pickup.tiny_key", "type": "pickup", "x": 9, "y": 4, "item_id": "item.tiny_key", "log": "你找到了一枚小金钥匙。"},
            {"id": "battle.card_guard", "type": "battle", "x": 10, "y": 5, "enemy_id": "enemy.card_guard", "once": True, "quest_id": "quest.follow_rabbit", "battle_background_asset_id": "battlebg.queen_court", "win_outcomes": [{"id": "key.path", "lines": [{"speaker": "爱丽丝", "text": "你们只是一副会排队的纸牌。"}], "set_flags": {"has_tiny_key": True}}]},
            {"id": "rest.mushroom", "type": "rest", "x": 11, "y": 6, "rest_point_id": "rest.mushroom"},
            {"id": "exit.to_tea", "type": "transfer", "x": 12, "y": 5, "target_map_id": "map.mad_tea_party", "target_x": 1, "target_y": 5},
        ],
    })
    write_json(maps / "mad_tea_party.map.json", {
        "id": "map.mad_tea_party", "title": "疯帽匠茶会", "width": 14, "height": 9, "asset_id": "map.mad_tea_party", "boundary_file": "../boundaries/mad_tea_party.boundaries.json",
        "layers": {"ground": make_path_layer(14, 9, tea, "path"), "collision": collision(14, 9)},
        "events": [
            {"id": "npc.hatter", "type": "npc", "x": 4, "y": 3, "name": "疯帽匠", "dialogue_id": "dialogue.hatter", "sprite_asset_id": "sprite.mad_hatter"},
            {"id": "npc.dormouse", "type": "npc", "x": 8, "y": 3, "name": "睡鼠", "dialogue_id": "dialogue.dormouse", "sprite_asset_id": "sprite.dormouse"},
            {"id": "battle.teapot", "type": "battle", "x": 8, "y": 5, "enemy_id": "enemy.mad_teapot", "once": True, "quest_id": "quest.unstick_time", "battle_background_asset_id": "battlebg.tea_table", "win_outcomes": [{"id": "time.unstuck", "lines": [{"speaker": "疯帽匠", "text": "杯子动了！时间终于肯换座位了。"}], "set_flags": {"time_unstuck": True}, "complete_quest_id": "quest.unstick_time"}]},
            {"id": "rest.tea_chair", "type": "rest", "x": 10, "y": 4, "rest_point_id": "rest.tea_chair"},
            {"id": "exit.back_garden", "type": "transfer", "x": 1, "y": 5, "target_map_id": "map.rabbit_hole_garden", "target_x": 11, "target_y": 5},
            {"id": "exit.to_court", "type": "transfer", "x": 12, "y": 5, "target_map_id": "map.queen_court", "target_x": 1, "target_y": 5},
        ],
    })
    write_json(maps / "queen_court.map.json", {
        "id": "map.queen_court", "title": "红心法庭", "width": 14, "height": 9, "asset_id": "map.queen_court", "boundary_file": "../boundaries/queen_court.boundaries.json",
        "layers": {"ground": make_path_layer(14, 9, court, "path"), "collision": collision(14, 9)},
        "events": [
            {"id": "npc.queen", "type": "npc", "x": 7, "y": 3, "name": "红心王后", "dialogue_id": "dialogue.queen", "sprite_asset_id": "sprite.queen_hearts"},
            {"id": "npc.court_echo", "type": "npc", "x": 10, "y": 4, "name": "法庭书记", "dialogue_id": "dialogue.court_echo", "sprite_asset_id": "sprite.white_rabbit"},
            {"id": "battle.queen", "type": "battle", "x": 11, "y": 5, "enemy_id": "enemy.queen_hearts", "once": True, "quest_id": "quest.overturn_trial", "battle_background_asset_id": "battlebg.queen_court", "win_outcomes": [{"id": "trial.overturned", "lines": [{"speaker": "红心王后", "text": "砍掉她的脑袋！"}, {"speaker": "爱丽丝", "text": "你们不过是一副纸牌。"}], "set_flags": {"trial_overturned": True, "ending:wake": True}, "complete_quest_id": "quest.overturn_trial"}]},
            {"id": "exit.back_tea", "type": "transfer", "x": 1, "y": 5, "target_map_id": "map.mad_tea_party", "target_x": 11, "target_y": 5},
        ],
    })
