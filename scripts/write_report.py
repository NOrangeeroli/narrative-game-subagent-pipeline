...
def write_final_report(run_root: Path) -> Path:
    ...
    adventure_validation = load_optional_json(path_for(run_root, "adventure_validation_report")) or {"status": "missing", "findings": []}
    ...
    payload = {
        ...
        "adventure": {
            "manifest": STAGE_PATHS["adventure_manifest"] if (run_root / STAGE_PATHS["adventure_manifest"]).exists() else None,
            "validation_report": STAGE_PATHS["adventure_validation_report"] if (run_root / STAGE_PATHS["adventure_validation_report"]).exists() else None,
